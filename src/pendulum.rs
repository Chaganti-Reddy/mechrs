...
mod tests {
    ...
    fn large_angle_period_longer_than_small_angle() {
        ...
        assert!(
            large > p.analytical_period(),
            "Large angle period ({:.4}) should exceed small-angle ({:.4})",
            large,
            p.analytical_period()
        );
    }
    ...
    fn nonlinear_vs_small_angle_diverge_at_large_amplitude() {
        ...
        let small_angle: Vec<f64> = p
            .small_angle_states(theta0, dt)
            .take(steps)
            .map(|(_, th)| th)
            .collect();

        let max_diff = nonlinear
            .iter()
            .zip(small_angle.iter())
            .map(|(a, b)| (a - b).abs())
            .fold(0.0_f64, f64::max);

        assert!(
            max_diff > 0.01,
            "60° amplitude should show measurable divergence: {max_diff}"
        );
    }
}
